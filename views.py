...
from flask import render_template, request, redirect, flash, jsonify, session as flask_session
from models import session as db_session, User, Plan
...
def get_planos():
    ...
    planos = db_session.query(Plan).filter_by(user_email=flask_session['user_email']).order_by(Plan.id.desc()).all()
...
def update_plano(plano_id):
    ...
    plano = db_session.query(Plan).filter_by(id=plano_id, user_email=flask_session['user_email']).first()
    if not plano:
